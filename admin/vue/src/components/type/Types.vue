<template>
  <div class="flex flex-row justify-between">
    <div class="">
      ...
      <div v-if="!types.loading" class="mt-10 flex flex-row justify-between">
        <div class="ml-10 font-bold  text-yellow-500 ">
          <p>Всего: {{!types.loading ? types.data.length : 0}}
      ...
      <div v-if="!types.loading" class="flex flex-col">
        <div class="overflow-x-auto ">
          <div class=" inline-block min-w-full">
            <div class="overflow-hidden">
              <TypeList @type="getType" :types="types" />
            </div>
          </div>
        </div>
      </div>
  ...
  <div v-if="types.loading" class=" text-center flex my-auto w-full h-full">
      ...
      <div class="flex flex-col">
        ...
        <div class="flex justify-end flex-row ">
          ...
          <div class="flex" v-if="model.id">
            <button type="button" @click="deleteType"  class="bg-red-700 mr-4 mb-5 rounded-xl p-3.5 mt-5 text-sm text-white font-bold hover:bg-red-800 hover:cursor-pointer focus:outline focus:outline-red-500">Удалить</button>
          </div>
        </div>
      </div>
    </form>
...
function saveType() {
  ...
  store.dispatch("saveType", { ...model.value }).then(() => {
    store.commit("notify", {
...
function deleteType() {
  if (
    ...
    store.dispatch("deleteType", model.value.id).then(() => {
      store.commit("notify", {
        type: 'success',
