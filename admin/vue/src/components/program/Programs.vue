<template>
  <div class="flex flex-row justify-between">
    <div class="">
      ...
      <div v-if="!programs.loading" class="mt-10 flex flex-row justify-between">
        <div class="ml-10 font-bold  text-yellow-500 ">
          <p>Всего: {{!programs.loading ? programs.data.length : 0}}
      ...
      <div v-if="!programs.loading" class="flex flex-col">
        <div class="overflow-x-auto ">
          <div class=" inline-block min-w-full">
            <div class="overflow-hidden">
              <ProgramList @program="getProgram" :programs="programs" />
            </div>
          </div>
        </div>
      </div>
  ...
  <div v-if="programs.loading" class=" text-center flex my-auto w-full h-full">
  ...
  <div v-if="show" class="absolute min-h-screen h-full inset-y-0 overflow-auto right-0 w-5/12 bg-secondary">
    <form @submit.prevent="saveProgram">
      <div class="flex flex-col">
        ...
        <div class="flex flex-col justify-around w-full mt-10">
          <div class="flex text-sm flex-col w-11/12 ml-5 mt-5 text-white">
            <label for="name" class="font-medium text-right">Код</label>
            <div class="flex flex-row mt-2 justify-end w-full">
              <div class="w-4/5">
                <input type="text" v-model="model.code" name="name" class="bg-input focus:ring focus:ring-indigo-600  text-sm w-full shadow rounded-xl border border-none text-white"
                       placeholder="Тип">
              </div>
            </div>
          </div>
        ...
        <div class="flex justify-end flex-row ">
          ...
          <div class="flex" v-if="model.id">
            <button type="button" @click="deleteProgram"  class="bg-red-700 mr-4 mb-5 rounded-xl p-3.5 mt-5 text-sm text-white font-bold hover:bg-red-800 hover:cursor-pointer focus:outline focus:outline-red-500">Удалить</button>
          </div>
        </div>
      </div>
    </form>
...
function saveProgram() {
  ...
  store.dispatch("saveProgram", { id: model.value.id, name: model.value.name, code: model.value.code }).then(() => {
    store.commit("notify", {
...
function deleteProgram() {
  if (
    ...
    store.dispatch("deleteProgram", model.value.id).then((res) => {
      store.dispatch("getPrograms")
      close()
    }).catch(() => {
      store.commit("notify", {
...
store.dispatch("getPrograms", {programData: model})
</script>
